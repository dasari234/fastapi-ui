...
export function Select<T extends object>({
  ...
  const getSelectedLabel = (): string => {
    if (!selected) return "";
    const selectedOption = options.find((option) => String(option.value) === String(selected));
      ...
      <div className="relative">
        ...
        >
          <input
            ...
            placeholder={isOpen && searchable ? "Search options..." : placeholder}
            className={cn(
                ...
                filteredOptions.map((option) => {
                  ...
                  return (
                    ...
                    >
                      {isSelected && tickPosition !== "only-tick" && (
                        <Check
                          className={cn(
                            ...
                            tickPosition === "left" ? "order-first" : "order-last"
                          )}
                          strokeWidth={3}
                        />
                      )}
                      ...
                      {isSelected && tickPosition === "only-tick" && (
                        <Check className="h-4 w-4 text-blue-600 ml-auto flex-shrink-0" strokeWidth={3} />
                      )}
                    </div>
                  );
                })
